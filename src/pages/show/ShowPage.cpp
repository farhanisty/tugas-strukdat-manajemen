...
void ShowPage::before() {
  ...
  for (auto product : products) {
    cout << "║ Code  : " << std::setw(35) << std::left << (product->code.empty() ? "-" : product->code) << " ║\n";
    cout << "║ Name  : " << std::setw(35) << std::left << (product->name.empty() ? "-" : product->name) << " ║\n";
    cout << "║ Price : " << std::setw(35) << std::left << (product->price == 0 ? "-" : std::to_string(product->price)) << " ║\n";
